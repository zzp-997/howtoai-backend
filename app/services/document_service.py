...
class DocumentService(BaseService[Document]):
    ...
    async def find_by_category(self, db: AsyncSession, category_id: int) -> List[Document]:
        ...
        result = await db.execute(
            select(Document).where(Document.categoryId == category_id).order_by(Document.createdAt.desc())
        )
    ...
    async def find_by_uploader(self, db: AsyncSession, user_id: int) -> List[Document]:
        """按上传者查询"""
        result = await db.execute(
            select(Document).where(Document.uploadBy == user_id).order_by(Document.createdAt.desc())
        )
        return result.scalars().all()
    ...
    async def search(self, db: AsyncSession, keyword: str) -> List[Document]:
        """搜索文档"""
        result = await db.execute(
            select(Document).where(Document.name.ilike(f"%{keyword}%")).order_by(Document.createdAt.desc())
        )
        return result.scalars().all()
